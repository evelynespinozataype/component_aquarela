...
def index():
	...
	if ('id_participante' in request.cookies):
		...
		return render_template('login_participante.html',permitido=oficina.is_open(), titulo_pagina="Aquarela Virtual", contador=detector_cor.count, nome_participante=participante['nome'],url_coracao='/static/images/avatar/coracao.png', coisa = participante['coisa'], avatar=get_avatar(participante['url_avatar']))
	else:
		return render_template('login_participante.html',permitido=oficina.is_open(), titulo_pagina="Aquarela Virtual", coisa='', url_coracao='',contador=detector_cor.count, nome_participante='',avatar='')
...
def quadro_desenho():
	if (request.method == 'POST'):
		if ('nome' in request.form and 'avatar' in request.form):
			...
			definicao_nome = nome_participante.split('.')
			if len(definicao_nome) > 1:
				if len(definicao_nome) > 2:
					print("Local "+ definicao_nome[0])
					local = definicao_nome[0]
					print("Coisa "+ definicao_nome[1])
					coisa = definicao_nome[1]
					nome_participante = definicao_nome[2]
					
			resposta.set_cookie('id_participante', str(id_participante), expires= data_expiracao_cookie)
			resposta.set_cookie('local', local, expires= data_expiracao_cookie)
			resposta.set_cookie('coisa', coisa, expires= data_expiracao_cookie)
			...
			oficina.gravar_log({'id':id_participante,'local':local, 'coisa':coisa, 'nome': nome_participante, 'url_avatar': url_avatar},'desenho',f'Fez login e iniciou os cookies com ({id_participante}) ({nome_participante}) e ({avatar})','')
			return resposta
	else:
		if ('id_participante' in request.cookies): #tratamento F5
			...
			liberar = True
			return render_template('folha_desenho.html', titulo_pagina="Aquarela Virtual", contador=detector_cor.count, url_coracao='/static/images/avatar/coracao.png', coisa = participante['coisa'], nome_participante = participante['nome'], url_avatar=participante['url_avatar'])
		oficina.gravar_log({'id':None, 'local':'', 'coisa':'', 'nome': '', 'url_avatar': ''},'desenho','Tentativa de acesso direto a URL /desenho sem login','')
		liberar = False
...
def joined_admin(data):
	...
	oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'tela admin', 'admin logou','')
...
def terminar_oficina(data):
	if oficina.terminou_oficina():
		oficina.set_fim_oficina(False)
		oficina.gravar_log({'id':'0000','local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela admin','Cancelou ou riniciou a oficina','')
		oficina.set_open()
	else:
		...
		if oficina.get_estrofe() == 0:
			finalizar_oficina()
			oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela admin','Encerrando a oficina pois não existe estrofe em andamento','')
		else:
			oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela admin','A oficina será encerrada após o fim da estrofe ' + str(oficina.get_estrofe()),'')
	emit('btn-encerrar',{'encerrar':oficina.terminou_oficina()},to='admin')
...
def selecionar_fotos(data):
	...
	oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela Admim','Escolhendo fotos para remoção','')
...
def mover_fotos(data):
	if len(foto.get_lista_fotos_remocao()) > 0:
		...
		oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela Admin','Removendo fotos','')
...
def finalizar_oficina():
	...
	oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela admim','Encerramento da oficina','')

@socket_io.on('renomear_oficina',namespace='/desenho')
def renomear_oficina(data):	
	foto.set_nome_oficina(oficina.get_sala())
	oficina.gravar_log({'id':'0000', 'local':'web', 'coisa':'None', 'nome': 'admin', 'url_avatar': ''},'Tela Admim','Renomeou a oficina. De ' + oficina.get_sala() + ' para ' + data['nome'],'')
	oficina.set_nome_oficina(data['nome'])

sid_arduino = ''
@socket_io.on('joined_remoto',namespace="/remoto")
def joined_arduino(data):
	global sid_arduino
	sid_arduino = request.sid
	print("--- SocketIo Remoto Concetado ---")
	emit('remote_aquarela',{"tag":"login_fisico","c":""},json=True,to=sid_arduino,namespace='/remoto')
	print(sid_arduino)

@socket_io.on('disconnect',namespace='/remoto')
def disconnect_remoto():
	global sid_arduino
	print("Fora do ar os físicos")
	sid_arduino = ''
...
def inicia_estrofe(data,participante):
	...
	#notifica_fisico_aquarela(oficina.get_objeto_int(data))
		...
		if not oficina.terminou_oficina():
			if ('id_participante' in request.cookies):
				...
				if oficina.verifica_emocao(data):
					notifica_fisico_emocao(data)
					emit('feedback-afetivo-social', {'participante': participante, 'estadoAfetivo': str(data)}, json=True, to=oficina.get_sala())
					registrar_log('Sistema', 'Sistema mostrou emoção e enviou para o Físico' + str(data), get_objeto_qrcode(str(data)))
					return
...
def get_participante():
	return {'id':request.cookies.get('id_participante'), 'local': request.cookies.get('local'), 'coisa': request.cookies.get('coisa'), 'nome': request.cookies.get('nome'), 'url_avatar': get_url_avatar(request.cookies.get('avatar'))}
...
@socket_io.on('login_obj',namespace="/remoto")
def login_objeto(data):
	print(data["coisa"][0])
	if(data["coisa"][0] == "A"):
		oficina.adicionar_fisico_aquarela(data["coisa"][0],{"tag":data["coisa"]})
	elif(data["coisa"][0] == "E"):
		oficina.adicionar_fisico_emocao(data["coisa"][0],{"tag":data["coisa"]})
	else:
		oficina.adicionar_fisico_aquarela(data["coisa"][0],{"tag":data["coisa"]})
		oficina.adicionar_fisico_emocao(data["coisa"][0],{"tag":data["coisa"]})


def notifica_fisico_emocao(info):
	lista_fisico = oficina.get_todos_fisico_emocao()
	for key in lista_fisico:
		print(lista_fisico[key])
		notifica_objeto(lista_fisico[key]["tag"],info)
		registrar_log('Sistema', 'Enviou para a Luminaria o valor de ' + str(info), get_objeto_qrcode(str(info)))


def notifica_fisico_aquarela(info):
	lista_fisico = oficina.get_todos_fisico_aquarela()
	for key in lista_fisico:
		print(lista_fisico[key])
		notifica_objeto(lista_fisico[key]["tag"],info)
		registrar_log('Sistema', 'Enviou para o quadro o valor de ' + str(info), get_objeto_qrcode('0'+str(info)))

def notifica_fisico(info):
	lista_fisico = oficina.get_todo_fisico()
	for key in lista_fisico:
		print(lista_fisico[key])
		notifica_objeto(lista_fisico[key]["tag"],info)

def notifica_objeto(tag,qrc):
	emit('remote_aquarela',{"tag":""+tag+"","qrc":qrc},json=True,to=sid_arduino,namespace='/remoto')

@socket_io.on('coracao',namespace="/remoto")
def coracao(data):
	print(data)
	emit('coracao_aquarela',data,json=True,to=oficina.get_sala(),namespace='/desenho')
	for pessoa in oficina.get_participantes_online('-1'):
		print(pessoa)
		if pessoa['coisa'] == data['coisa']:
			print(pessoa['coisa'])
