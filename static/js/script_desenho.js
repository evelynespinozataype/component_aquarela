...
socket.on('coracao_aquarela', coracao =>{
  print(coracao);
...
adicionaParticipanteNaLista = participante => {
  if(naoLogado(participante.id)){
        ...
        //<img src="coracao.png" id="idHeart1" class="heart"></img>
        ...
        //if(participante.coisa != '') coracaoBatendo(participante.coisa,60);
    }
...
socket.on('participante-presente', participante => {
  if (eu_logado.id != participante.id){
    ...
    console.log("--------------------------------");
    //document.getElementById(participante.id). '<div id="idHeart" class="heart">&#x2665;</div>';
});
...
var at = 0;
function coracaoBatendo(_id, tempo){
  console.log('...........Id ' + _id + ' Tempo ' + tempo);
  at = (1/(tempo/30));
  document.getElementById(_id).style.animationDuration = at+"s";   
}
