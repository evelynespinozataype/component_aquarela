...
class Log:
    ...
    def registrar_logCSV(self, participante, sala, interface, descricao, objeto):
        ...
        with open(arquivo_log, 'w', encoding="utf-8", errors="ignore", newline='') as csvfile:
            fieldnames = ['DATA','HORA','NOME_OFICINA','NOME_PARTICIPANTE','ID_PARTICIPANTE','LOCAL','COISA','INTERFACE','DESCRIÇÃO','OBJETO']
    ...
    def __criar_arquivo_log(self,sala):
        ...
        if(not os.path.isfile(arquivo_log)):
            with open(arquivo_log, 'a', encoding="utf8") as logfile:
                logfile.write("Data;Hora;Oficina;ID Participante;Local;Coisa;Nome Participante;Avatar;Interface;Descrição;Objeto")
        return arquivo_log
    ...
    def registrar_log(self, participante, sala,interface, descricao, objeto):
        ...
        with open(arquivo_log, 'a', encoding="utf8") as logfile:
                ...
                logfile.write(";")
                logfile.write(participante['local'])
                logfile.write(";")
                logfile.write(participante['coisa'])
                logfile.write(";")
